...
import { SITE_FAQ, splitFaq } from "@/lib/faq";
import { OG_IMAGES, TWITTER_IMAGES } from "@/lib/seo";
import { faqJsonLd, landingJsonLd, serialiseJsonLdAll } from "@/lib/structured-data";
...
export default function LandingPage() {
  /**
   * "What is Edgify?" becomes the fact block under the hero; the other seven questions become
   * the FAQ section further down. Splitting rather than writing the fact block's copy inline
   * keeps exactly one copy of that answer in the repository — the one the FAQ markup quotes.
   */
  const { lead, rest } = splitFaq(SITE_FAQ);
      ...
      */}
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: serialiseJsonLdAll([
            landingJsonLd(env.SITE_URL),
            // Every question and answer below is rendered on this page — the fact block under the
            // hero carries the first, the FAQ section carries the rest (lib/faq.ts `splitFaq`).
            faqJsonLd(env.SITE_URL, "/", SITE_FAQ),
          ]),
        }}
      />
      ...
      <div className="lx-wrap">
        ...
        {/*
          ...
          It adds no styling: `<main>` is a block box and every child of `.lx-wrap` it now
          contains was already a full-width block. `app/(marketing)/landing.test.tsx` compares
          rendered CLASS TOKENS against the prototype and this element carries none, so the port's
          fidelity check is unaffected.
        */}
        <main>
          ...
          {/* The direct, quotable statement of what Edgify is — components/key-answer.tsx. */}
